...
defineFeature(comment_feature, (test) => {
  test("Criar um comentário", ({ given, when, and, then }) => {
    ...
    then(/^O comentário deve ser adicionado ao post$/, async () => {
      ...
      expect(
        response.data.some(
          (comment) => comment.comment_id === comment_test.comment_id,
        ),
      ).toBe(true);
    });
  });
  ...
  test("Atualizar um comentário", ({ given, when, and, then }) => {
    ...
    and(
      /^Eu já publiquei um comentário com o texto "(.*)" respondendo o primeiro "(.*)" review$/,
      async (arg0, arg1) => {
        post_id = filme_data.posts[Number(arg1)].post_id;
  ...
  test("Criar um comentário de comentário", ({ given, when, and, then }) => {
    ...
    and(
      /^aperto no botão "(.*)" do primeiro "(.*)" comentario da primeira "(.*)" review que tinha escrito "(.*)"$/,
  ...
  test("Erro ao apagar um comentário de outro usuário", ({
    given,
    when,
    and,
    then,
    ...
    and(
      /^O usuário "(.*)" publicou um comentário com o texto "(.*)" respondendo o primeiro "(.*)" review$/,
    ...
    when(/^Eu clico no botão "(.*)"$/, async (arg0) => {
      ...
      try {
        await axios.delete("http://localhost:3000/comment", {
          data: delete_req,
        });
      } catch (error) {
        erro = error.response.data.error;
