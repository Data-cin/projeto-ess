...
defineFeature(comment_feature, (test) => {
  test("Adicionar plataforma", ({ given, when, and, then }) => {
    ...
    and(
      /^Eu preencho "(.*)" no campo "(.*)", "(.*)" no campo "(.*)", "(.*)" no campo "(.*)" e clico em"(.*)"$/,
      async (nome, arg1, url, arg3, image, arg5, arg6) => {
        try {
    ...
    and(
      /^Eu posso ver a plataforma "(.*)" na lista de plataformas disponíveis$/,
      async (nomePlataforma) => {
        try {
          ...
          const plataforma = filme.plataformas.some(
            (platform) => platform.nome === nomePlataforma,
          );
  ...
  test("Remover plataforma", ({ given, when, and, then }) => {
    ...
    then(/^A plataforma "(.*)" é removida$/, async (nomePlataforma) => {
      try {
        ...
        const response = await axios.delete("http://localhost:3000/platform", {
          data: platform_test,
        });
    ...
    and(
      /^Eu não vejo a plataforma "(.*)" na lista de plataformas disponíveis$/,
      async (nomePlataforma) => {
        try {
          // Fazendo a requisição GET para obter as plataformas do filme
          const response = await axios.get("http://localhost:3000/");
  ...
  test("Adicionar plataforma sem preencher todos os campos", ({
    given,
    when,
    and,
    then,
    ...
    and(
      /^Eu preencho "(.*)" no campo "(.*)", "(.*)" no campo "(.*)" e clico em "(.*)"$/,
      async (url, arg1, img, arg3, arg5) => {
        try {
          // Fazendo a requisição POST
          const platform_test = {
            filme_id: filme_id,
