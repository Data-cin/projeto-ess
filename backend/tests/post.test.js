...
defineFeature(post_feature, (test) => {
  test("Criar um review.", ({ given, and, then }) => {
    ...
    given(
      /^Execute um get na rota "(.*)" armazenando o id do filme "(.*)"$/,
      async (arg0, arg1) => {
    ...
    and(
      /^Execute um post na rota "(.*)" com o filme_id no campo filme_id, "(.*)" no campo user_id, "(.*)" no campo nota e "(.*)" no campo review$/,
    ...
    then(
      /^Execute um get na rota "(.*)" e verifique se um filme com mesmo post_id existe no filme$/,
      async (arg0) => {
        // Fazendo a requisição GET
  ...
  test("Falha criação de review sem nota.", ({ given, when, and, then }) => {
    ...
    and(
      /^Execute um POST na rota "(.*)" com o filme_id no campo filme_id, "(.*)" no campo user_id e "(.*)" no campo review, deixando o campo nota vazio.$/,
    ...
    and(
      /^Verifique se o status code é (\d+) e se a mensagem de erro é "(.*)".$/,
      (arg0, arg1) => {
    ...
    then("Verifique se o review com o filme_id não foi criado.", async () => {
      ...
      const reviewExists = response.data.some(
        (review) => review.filme_id === filme_id && review.user_id === 1338,
      );
  ...
  test("Atualizar um review.", ({ given, and, when, then }) => {
    ...
    given(
      ...
      async (arg0, arg1) => {
        const response = await axios.get("http://localhost:3000/");
        filme_id = response.data.find(
          (filme) => filme.nome === "Oppenheimer",
        ).filme_id;
      },
    );

    and(
      /^Execute um POST na rota "(.*)" com o filme_id no campo filme_id, "(.*)" no campo user_id, "(.*)" no campo nota e "(.*)" no campo review e armazene o post_id.$/,
      async (arg0, arg1, arg2, arg3) => {
        const postReview = {
          user_id: 1336,
          filme_id: filme_id,
    ...
    when(
      /^Execute um PUT na rota "(.*)" com "(.*)" no campo review e "(.*)" no campo nota.$/,
      async (arg0, arg1, arg2) => {
        const putReview = {
          filme_id: filme_id,
          post_id: post_id,
    ...
    then(
      /^Execute um GET na rota "(.*)" e verifique se o review atualizado possui o texto "(.*)" e a nota "(.*)".$/,
      async (arg0, arg1, arg2) => {
        const response = await axios.get(arg0);
  ...
  test("Criar um review sem a descrição.", ({ given, and, then }) => {
    ...
    and(
      /^Execute um post na rota "(.*)" com o filme_id no campo filme_id, "(.*)" no campo user_id e "(.*)" no campo nota.$/,
      async (arg0, arg1, arg2) => {
        // Fazendo a requisição POST
        const post_test = {
          user_id: arg1,
  ...
  test("Apagar um review", ({ given, and, when, then }) => {
    ...
    when(
      /^Executa um DELETE na rota "(.*)" para apagar o review criado.$/,
      async (arg0) => {
        const post_test = {
          post_id: post_id,
    ...
    and(
      /^Verifica se o status da resposta do DELETE é (\d+) \(indicando remoção bem-sucedida\).$/,
      (arg0) => {
        // Verificando se o status code é 201
        expect(response.status).toBe(Number(arg0));
      },
    ...
    then(
      /^Executa um GET na rota "(.*)" para verificar se o review foi apagado.$/,
      async (arg0) => {
        // Fazendo a requisição GET
        response = await axios.get(arg0);
        // Busca o filme e o post específicos
